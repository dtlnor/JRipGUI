...
匹配这些字符串：MMX2，SSE，SSE2，SSE3，SSSE3，SSE4，SSE4.1，SSE4.2，AVX，XOP，
               FMA4，AVX2，FMA3
...
并发编码帧的数量。使用单帧线程可以略微改善压缩，
因为整个参考帧始终可用于运动补偿，但它严重影响性能。
...
调整特定类型的源或情境的设置。更改将在--preset之后但在所有其他参数之前应用。
默认无
...
虽然这允许更深入的集成，但应用程序的责任是：（a）确保分配的图片具有额
外的填充空间，这将由库完成；（b）缓冲区在库具有之前不会被回收完成对此
帧的编码（可以通过跟踪由x265输出的NAL来计算）
...
3.i444 (4:4:4) - Supported by Main 4:4:4, Main 4:4:4 10, Main 4:4:4 12,
  Main 4:4:4 16 Intra profiles
4.nv12
5.nv16''',

'Chinese': '''--input-csp <integer|string>
色度子采样（仅限YUV）：此时仅支持4：0：0（单色），4：2：0，4：2：2和4：4：4。
输入的色度子采样格式必须与您所需的输出色度子采样格式匹配（libx265不会执行任何
色度子采样转换），并且必须由您指定的HEVC配置文件支持。

0.i400（4：0：0单色） - Main或Main10配置文件不支持
1.i420（默认值为4：2：0） - 所有HEVC配置文件均支持
2.i422（4：2：2） - Main，Main10和Main12配置文件不支持
3.i444（4：4：4） - 被Main 4:4:4, Main 4:4:4 10, Main 4:4:4 12,
  Main 4:4:4 16 Intra支持
4.NV12
5.nv16'''},
...
The source dimensions must be field dimensions and the FPS must be in
 units of fields per second. The decoder must re-combine the fields in
  their correct orientation for display.''',

'Chinese': '''--interlace <false|tff|bff>, --no-interlace
0.逐行扫描（默认）
1.TFF
2.BFF
HEVC将隔行内容编码为字段。必须以正确的时间顺序将字段提供给编码器。
源维度必须是字段维度，FPS必须以每秒字段为单位。
解码器必须以正确的方向重新组合字段以进行显示。'''},
...
Bitstream output file name. If there are two extra CLI options,
the first is implicitly the input filename and the second is the
output filename, making the --output option optional.
...
比特流输出文件名。如果有两个额外的CLI选项，第一个是隐式输入文件名，
第二个是输出文件名，使--output选项可选。
...
If the requested bit depth is not the bit depth of the linked libx265, it will attempt 
to bind libx265_main for an 8bit encoder, libx265_main10 for a 10bit encoder, or libx265_
main12 for a 12bit encoder, with the same API version as the linked libx265.

If the output depth is not specified but --profile is specified,
the output depth will be derived from the profile name.''',

'Chinese': '''--output-depth, -D 8|10|12
输出HEVC比特流的位深，也是编码器的内部比特深度。
如果请求的位深度不是链接的libx265的位深度，
它将尝试绑定libx265_main用于8位编码器，
libx265_main10用于10位编码器，
或libx265_main12用于12位编码器，
其API版本与链接的libx265相同。

如果未指定输出深度但--profile指定了输出深度，
则将从配置文件名称派生输出深度。'''},

'chunk-start':
{'English': '''--chunk-start <integer>
First frame of the chunk. Frames preceeding this in display 
order will be encoded, however, they will be discarded in the bitstream. 
This feature can be enabled only in closed GOP structures. 
Default 0 (disabled).''',

'Chinese': '''--chunk-start <integer>
块的第一帧。将按照显示顺序在此之前的帧进行编码，但是，它们将在比特流中被丢弃。
只能在封闭的GOP结构中启用此功能。
默认值为0（禁用）。'''},

'chunk-end':
{'English': '''--chunk-end <integer>
Last frame of the chunk. Frames following this in display order will 
be used in taking lookahead decisions, but, they will not be encoded. 
This feature can be enabled only in closed GOP structures. 
Default 0 (disabled).''',

'Chinese': '''--chunk-end <integer>
块的最后一帧。显示顺序后面的帧将用于做出先行决定，但是，它们不会被编码。
只能在封闭的GOP结构中启用此功能。
默认值为0（禁用）。'''},
...
ensuring the output stream will be decodable by a 
decoder which supports that profile. May abort the 
encode if the specified profile is impossible to be 
supported by the compile options chosen for the encoder 
(a high bit depth encoder will be unable to output 
...
强制执行指定配置文件的要求，确保输出流可由支持该配置文件的解码器解码。
如果为编码器选择的编译选项不能支持指定的配置文件，则可以中止编码（高位
深度编码器将无法输出符合Main或MainStillPicture的比特流）。
...
Minimum decoder requirement level. Defaults to 0, 
which implies auto-detection by the encoder. 
If specified, the encoder will attempt to bring the 
encode specifications within that specified level. 
If the encoder is unable to reach the level it issues 
a warning and aborts the encode. If the requested 
requirement level is higher than the actual level, 
the actual requirement level is signaled.

Beware, specifying a decoder level will force the 
encoder to enable VBV for constant rate factor encodes, 
which may introduce non-determinism.

The value is specified as a float or as an integer with 
the level times 10, for example level 5.1 is specified as 
“5.1” or “51”, and level 5.0 is specified as “5.0” or “50”.

Annex A levels: 1, 2, 2.1, 3, 3.1, 4, 4.1, 5, 5.1, 5.2, 6, 
6.1, 6.2, 8.5''',

'Chinese': '''--level-idc <integer|float>
最低解码器要求级别。默认为0，表示编码器自动检测。
如果指定，编码器将尝试将编码规范置于该指定级别内。
如果编码器无法达到该级别，则会发出警告并中止编码。
如果请求的需求级别高于实际级别，则发信号通知实际
需求级别。

请注意，指定解码器级别将迫使编码器启用VBV以进行
恒定速率因子编码，这可能会造成非确定性。

该值指定为浮点数或10倍整数，例如级别5.1指定为
“5.1”或“51”，级别5.0指定为“5.0”或“50”。

附件A级别：1,2,2.1,3,3.1,4,4.1,5,5.1,5.2,6,
6.1,6.2,8.5'''},

'high-tier':
{'English': '''--high-tier, --no-high-tier
If --level-idc has been specified, –high-tier allows 
the support of high tier at that level. The encoder 
will first attempt to encode at the specified level,
main tier first, turning on high tier only if necessary 
and available at that level.If your requested level does 
not support a High tier, high tier will not be supported. 
If –no-high-tier has been specified, then the encoder 
will attempt to encode only at the main tier.

Default: enabled''',

'Chinese': '''--high-tier, --no-high-tier
如果--level-idc已指定，则-high-tier允许在该级别支持high级别。
编码器将首先尝试在指定级别进行编码，首先是main级别，仅在必要时
在该级别使用high级别。如果您请求的级别不支持high级别，则不支持
high级别。如果指定了-no-high-tier，则编码器将尝试仅在main级别
进行编码。
...
'ref':
{'English': '''--ref <1..16>
Max number of L0 references to be allowed. This number 
has a linear multiplier effect on the amount of work 
performed in motion search, but will generally have a 
beneficial affect on compression and distortion.

Note that x265 allows up to 16 L0 references but the 
HEVC specification only allows a maximum of 8 total 
reference frames. So if you have B frames enabled only 
7 L0 refs are valid and if you have --b-pyramid enabled 
(which is enabled by default in all presets), 
then only 6 L0 refs are the maximum allowed by the HEVC specification. 
If x265 detects that the total reference count is greater than 8, 
it will issue a warning that the resulting stream is non-compliant 
and it signals the stream as profile NONE and level NONE and will 
abort the encode unless --allow-non-conformance it specified. 
Compliant HEVC decoders may refuse to decode such streams.

Default 3''',

'Chinese': '''--ref <1..16>
允许的最大L0引用数。此数字对运动搜索中执行的工作量具有线性乘数效应，
但通常会对压缩和失真产生有益影响。

请注意，x265最多允许16个L0参考，但HEVC规范仅允许最多8个参考帧。
因此，如果启用了B帧，则只有7个L0引用有效且如果已--b-pyramid启用
（默认情况下在所有预设中启用），则只有6个L0引用是HEVC规范允许的最大值。
如果x265检测到总引用计数大于8，它将发出警告，指出结果流不符合要求，
并且它将信号通知为配置文件NONE和level NONE，并且除非
--allow-non-conformance指定，否则将中止编码。
兼容的HEVC解码器可以拒绝解码这样的流。
...
By default it will abort any encode which does not meet strict 
level compliance. The two most likely causes for non-conformance 
are --ctu being too small, --ref being too high, or the bitrate or 
resolution being out of specification.

Default: disabled''',

'Chinese': '''--allow-non-conformance, --no-allow-non-conformance
允许libx265生成具有配置文件和级别NONE的比特流。默认情况下，它将中止任何
不符合严格级别合规性的编码。导致不合规的两个最可能的原因 --ctu太小，--ref太高，
或者比特率或分辨率超出规范。
...
If specified with incompatible encoding options, 
the encoder will attempt to modify/set the right 
encode specifications. 
If the encoder is unable to do so, this option 
will be turned OFF.
Highly experimental.
...
'Chinese': '''--uhd-bd
启用超高清蓝光格式支持。如果使用不兼容的编码选项，
编码器将尝试修改/设置正确的编码规范。如果编码器无
法执行此操作，则此选项将关闭。
高度实验性。
...
'rd':
{'English': '''--rd <1..6>
Level of RDO in mode decision. The higher the value, 
the more exhaustive the analysis and the more rate 
distortion optimization is used. The lower the value 
the faster the encode, the higher the value the smaller
the bitstream (in general). Default 3

Note that this table aims for accuracy, but is not 
necessarily our final target behavior for each mode.
...
Options which affect the coding unit quad-tree, 
sometimes referred to as the prediction quad-tree.''',

'Chinese': '''--rd <1..6>
模式决策中的RDO级别。值越高，分析越详尽，使用的失真优化越多。
值越低，编码越快，值越高，比特流越小（通常）。默认3
...
giving large reductions in bitrate. However this comes at a loss 
of parallelism with fewer rows of CUs that can be encoded in parallel, 
and less frame parallelism as well. Because of this the faster presets 
use a CU size of 32. 
Default: 64''',

'Chinese': '''--ctu, -s <64|32|16>
最大CU尺寸（宽度和高度）。最大CU尺寸越大，x265可以更有效地编码图像的平坦区域，
从而大大降低了比特率。然而，这导致并行性的损失，可以并行编码的CU行数较少，
并且帧并行性也较少。因此，更快的预设使用CU大小32。
默认值：64'''},
...
最小CU尺寸（宽度和高度）。通过使用16或32，编码器将不会分析低于该最小阈值的CU的成本，
从而在可预测的比特率增加的情况下节省大量计算。此设置对更快预设的性能有很大影响。
...
only use the reference(s) selected by 2Nx2N.

For all non-zero values of limit-refs, the current depth will 
evaluate intra mode (in inter slices), only if intra mode was 
chosen as the best mode for atleast one of the 4 sub-blocks.

You can often increase the number of references you are using 
(within your decoder level limits) if you enable one or both 
of these flags.

Default 3.''',

'Chinese': '''--limit-refs <0|1|2|3>
当设置为X265_REF_LIMIT_DEPTH（1）时，x265将根据用于编码下一个深度处
的4个子块的参考来限制在当前深度处分析的参考。例如，16x16 CU仅使用用于
编码其四个8x8 CU的引用。

当设置为X265_REF_LIMIT_CU（2）时，矩形和非对称分区将仅使用由2Nx2N运
动搜索选择的参考（包括在最低深度，否则不受深度限制影响）。

当设置为3（X265_REF_LIMIT_DEPTH && X265_REF_LIMIT_CU），在各深度
2Nx2N的运动搜索将只使用的引用从分割的CU和rect/amp运动搜索在该深度处
将只使用由2Nx2N的选择的参考（或多个）。

对于limit-ref的所有非零值，当前深度将评估帧内模式（在片间），仅当帧内
模式被选择为4个子块中的至少一个的最佳模式时。

如果启用这些标志中的一个或两个，通常可以增加正在使用的引用数（在解码器
级别限制内）。
...
When enabled, limit-modes will limit modes analyzed for each CU 
using cost metrics from the 4 sub-CUs. When multiple inter modes 
like --rect and/or --amp are enabled, this feature will use motion 
cost heuristics from the 4 sub-CUs to bypass modes that are unlikely 
to be the best choice. This can significantly improve performance 
when rect and/or --amp are enabled at minimal compression efficiency 
loss.''',

'Chinese': '''--limit-modes, --no-limit-modes
启用后，将使用来自4个子CU的成本度量来限制针对每个CU分析的模式。
当启用多个帧间模式--rect 和/或--amp启用时，此功能将使用来自4
个子CU的运动成本启发法来绕过不太可能是最佳选择的模式。当rect
和/或--amp以最小的压缩效率损失启用时，这可以显着改善性能。'''},

'rect':
{'English': '''--rect, --no-rect
Enable analysis of rectangular motion partitions Nx2N and 
2NxN (50/50 splits, two directions). 
Default disabled''',

'Chinese': '''--rect, --no-rect
启用矩形运动分区Nx2N和2NxN（50/50分割，两个方向）的分析。
默认禁用'''},
...
在1级时，速​​率 - 失真成本用于找到每个级别的最佳值（并允许psy-rdoq有效）。
它从预定量系数中消除了系数的信令成本与其反后量化失真之间的关系。
当 --psy-rdoq启用时，该式偏向于更多的能量中的残余（较大系数的绝对水平）

在2级时，速率 - 失真成本用于对每个4x4编码组进行抽取决策，包括在组位图内发信
号通知组的成本。如果不发信号通知整个编码组的总失真小于速率成本，
则该块被抽取。接下来，它将速率 - 失真成本分析应用于最后的非零系数，这可导致
许多（或所有）编码组被抽取。当RDOQ处于2级时，Psy-rdoq在保存能量
方面效果较差，因为它只影响水平失真成本。'''},
...
Level 3 - based on the average depth of the co-located and the neighbor CUs’ TU depth, 
limits recursion of the current CU.

Level 4 - uses the depth of the neighbouring/ co-located CUs TU depth to limit the 1st 
subTU depth. The 1st subTU depth is taken as the limiting depth for the other subTUs.

Enabling levels 3 or 4 may cause a mismatch in the output bitstreams between option:
–analysis-save and option:–analysis-load as all neighbouring CUs TU depth may not be 
available in the option:–analysis-load run as only the best mode’s information is 
available to it.
...
级别4 - 使用邻近/共同定位的CU TU深度的深度来限制第一subTU深度。
第一个subTU深度作为其他subTU的限制深度。

启用级别3或4可能会导致选项之间的输出比特流不匹配： –analysis-save和选项： –analysis-load，
因为所有相邻的CU TU深度可能在选项中不可用：–analysis-load运行仅为最佳模式的信息可用。
...
For this option to be effective with 32x32 intra CUs, --tu-intra-depth 
must be at least 2. For it to be effective with 64x64 intra CUs, 
--tu-intra-depth must be at least 3.
...
that the encoder may consider for merging motion predictions.
If a merge candidate results in no residual, it is immediately selected as a “skip”.
Otherwise the merge candidates are tested as part of motion estimation when searching 
for the least cost inter option.
The max candidate number is encoded in the SPS and determines the bit cost of signaling 
merge CUs.
Default 2''',
...
interpolation half-length (4) minus maximum subpel distance (2) minus 
one extra pixel just in case the hex search method is used.
If the search range were any larger than this, another CTU row of 
...
This enables the use of the motion vector from the collocated 
block in the previous frame to be used as a predictor.
Default is enabled''',
...
Influence rate distortion optimized quantization by favoring 
higher energy in the reconstructed image.
This generally improves perceived visual quality at the cost 
of lower quality metric scores.
It only has effect when --rdoq-level is 1 or 2. High values 
can be beneficial in preserving high-frequency detail.
...
Max intra period in frames. A special case of infinite-gop
(single keyframe at the beginning of the stream) can be 
triggered with argument -1. Use 1 to force all-intra. 
When intra-refresh is enabled it specifies the interval 
between which refresh sweeps happen.
Default 250''',
...
'ctu-info':
{'English': '''--ctu-info <0, 1, 2, 4, 6>
This value enables receiving CTU information asynchronously and determine 
reaction to the CTU information. Default 0. 
...
has changed, merge/skip otherwise. This option should be enabled only when planning 
to invoke the API function x265_encoder_ctu_info to copy ctu-info asynchronously. 
If enabled without calling the API function, the encoder will wait indefinitely.''',

'Chinese': '''--ctu-info <0, 1, 2, 4, 6>
该值允许异步接收CTU信息并确定对CTU信息的反应。默认值0. 
1：如果存在CTU信息，则强制分区。
2：（1）的功能，如果CTU信息已经改变，则减少qp。
4：当CTU信息改变时，（1）的功能和强制帧间模式，否则合并/跳过。
仅当计划调用API函数x265_encoder_ctu_info以异步复制ctu-info时，
才应启用此选项。如果在未调用API函数的情况下启用，编码器将无限期地等待。'''},

'intra-refresh':
{'English': '''--intra-refresh
...
and thereby refresh the image but over a period of multiple frames 
instead of a single keyframe.''',

'Chinese': '''--intra-refresh
启用定期帧内刷新（PIR）而不是关键帧插入。
PIR可以通过在非关键帧中插入一列帧内块来替换关键帧，
这些帧通过视频从一侧移动到另一侧，从而刷新图像，
但是在多帧的时间内而不是单个关键帧。'''},

'rc-lookahead':
{'English': '''--rc-lookahead <integer>
Number of frames for slice-type decision lookahead 
(a key determining factor for encoder latency). 
The longer the lookahead buffer the more accurate 
scenecut decisions will be, and the more effective 
cuTree will be at improving adaptive quant. Having 
a lookahead larger than the max keyframe interval 
is not helpful. Default 20

Range of values: Between the maximum consecutive 
bframe count (--bframes) and 250''',

'Chinese': '''--rc-lookahead <integer>
切片类型决策前瞻的帧数（编码器延迟的关键决定因素）。
前瞻缓冲区越长，场景切割决策就越准确，cuTree在提高
自适应量化方面的效果就越好。预测大于最大关键帧间隔是没有用的。
默认为20

值范围：在最大连续bframe计数（--bframes）和250之间'''},

'gop-lookahead':
{'English': '''--gop-lookahead <integer>
Number of frames for GOP boundary decision lookahead. 
If a scenecut frame is found within this from the gop 
boundary set by –keyint, the GOP will be extented until 
such a point, otherwise the GOP will be terminated as 
set by –keyint. Default 0.

Range of values: Between 0 and (–rc-lookahead - mini-GOP length)

It is recommended to have –gop-lookahaed less than –min-keyint 
as scenecuts beyond –min-keyint are already being coded as keyframes.''',

'Chinese': '''--gop-lookahead <integer>
GOP边界决策前瞻的帧数。如果场景切换帧中此从GOP边界通过设定发现-keyint中，
GOP将被推广的，直到这样的点，否则GOP将被终止，通过设置-keyint。默认值为0。

值范围：介于0和（-rc-lookahead - mini-GOP长度）之间

建议将-gop-lookahaed小于-min-keyint，因为-min-keyint之外的 场景切换已
被编码为关键帧。'''},
...
if __name__ == '__main__':
    ...
    for key, value in helps.items():
        new[key] = {}
        chi = value['Chinese'].splitlines()
        eng = value['English'].splitlines()
        for m, i in enumerate([eng, chi]):
            result = []
            for n, line in enumerate(i):
                if n is 0:
                    ...
                    result.append('\n')
            if m is 0:
                new[key]['English'] = ''.join(result)
            else:
                new[key]['Chinese'] = ''.join(result)
    print(new)
    pyperclip.copy(str(new))